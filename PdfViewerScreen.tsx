...
import type { WebView as WebViewType } from 'react-native-webview'; // WebViewType은 현재 사용되지 않지만, 타입 정의를 위해 남김.
...
const PdfViewerScreen: React.FC = () => {
  ...
  const pickPdf = async () => {
    try {
      ...
      if (result.assets && result.assets.length > 0) {
        ...
        console.log('선택한 PDF:', selectedUri);

        const fileBase64 = await FileSystem.readAsStringAsync(selectedUri, {
          encoding: FileSystem.EncodingType.Base64,
        });

        setBase64Pdf(fileBase64);
        setPdfUri(selectedUri);
        // Alert.alert('PDF 로드 시작', 'PDF 파일을 로드합니다.');
      } else {
  ...
  const onWebViewMessage = (event: any) => {
    try {
      ...
      if (data.type === 'webviewReady') {
        ...
        console.log('WebView reports ready!'); // WebView 준비 로그 추가
      } else if (data.type === 'error') {
        setError('WebView에서 PDF 렌더링 중 오류: ' + data.message); // 오류 메시지 명확화
        console.error('WebView 렌더링 오류:', data.message);
      } else if (data.type === 'success') { // 성공 메시지 처리 추가
        console.log('PDF 성공적으로 렌더링됨.');
        // Alert.alert('성공', 'PDF 파일이 성공적으로 로드되었습니다.'); // 필요 시 알림
      }
    } catch (e) {
      console.warn('웹뷰 메시지 파싱 오류', e);
    }
  };

  useEffect(() => {
    if (webViewReady && base64Pdf && webViewRef.current) {
      console.log('useEffect: PDF 데이터를 WebView로 전송 중...'); // 전송 로그 추가
      webViewRef.current.postMessage(
        JSON.stringify({ type: 'loadPdf', base64Pdf })
      );
    }
  ...
  return (
    <View style={styles.container}>
      ...
      {loading && <ActivityIndicator style={styles.indicator} size="large" color="#0000ff" />}
      {error && <Text style={styles.errorText}>{error}</Text>}
      <View style={styles.pdfContainer}>
        <WebView
          ref={webViewRef}
          source={{ html: webViewHtml }} // 이제 이 html이 실제 내용을 가집니다.
          originWhitelist={['*']}
          /*allowsFileAccess={true}*/ // 파일 접근 권한 활성화
          allowFileAccessFromFileURLs={true} // 파일 URL에서 파일 접근 활성화
          allowUniversalAccessFromFileURLs={true} // 유니버설 파일 접근 활성화
          javaScriptEnabled
          domStorageEnabled
          onMessage={onWebViewMessage}
          onLoadStart={() => console.log('WebView 로드 시작')}
          onLoadEnd={() => console.log('WebView 로드 완료 (HTML 로드됨)')}
          onError={(syntheticEvent) => {
            const { nativeEvent } = syntheticEvent;
            console.warn('WebView 일반 오류 (HTML 로드 중): ', nativeEvent);
            setError('WebView 로딩 중 오류: ' + nativeEvent.description);
          }}
          onHttpError={(syntheticEvent) => {
            const { nativeEvent } = syntheticEvent;
            console.warn('WebView HTTP 오류 (HTML 로드 중): ', nativeEvent);
